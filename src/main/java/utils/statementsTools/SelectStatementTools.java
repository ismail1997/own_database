...
public class SelectStatementTools {
	...
	public static void validSelectStatementOld(String statement)  throws Exception{
	...
	public static void validSelectStatement(String statement) throws Exception {
	    ...
	    if(Tools.countFrequencies(dividedSelect, "where")>1) {
	    	System.out.println("ERROR : you have error in your sql statement; 'where' keyword is repeated");return;
	    }
		...
		//if there is where clause but no and and limit , then we should get the field that passed as parameter
	    String fieldThatPassedForWhere="";
	    if(containsWhereClause && !containsWhereClauseAndAlso && !containsLimit) {
	    	fieldThatPassedForWhere=chosenSelect.substring(chosenSelect.indexOf("where")+"where".length(),chosenSelect.indexOf(";"));
		...
		if(containsAsterix && containsWhereClause && !containsWhereClauseAndAlso && !containsLimit) {
			TableTools.readDataFromTableWithAsterixWithWhereNoAndNoLimit(tableName,currentDatabase,fieldAndValue[0].trim(),fieldAndValue[1].trim());
		}
	}
}
